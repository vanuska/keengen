...
| «Настройка входа» | Да (SSH-профиль) | Нет — кнопка **серая** (SSH с роутера не нужен) |
...
**Для SSH к роутеру** нужны логин, пароль **или** SSH-ключ и порт **Dropbear Entware** — те, что задавали при установке XKeen / Entware. Типично: пользователь `root`, порт `22` (это не порт KeeneticOS `:2222`).

Большинству достаточно логина и пароля Entware в форме «Настройка входа». Альтернатива — пустой пароль + env `KEENGEN_SSH_KEY` (путь к приватному ключу): пошагово в [howto — Вход: пароль или SSH-ключ](docs/howto.md#вход-пароль-или-ssh-ключ). keengen на ПК ходит **только на private IP**.
...
«**Настройка входа**» → название профиля, адрес LAN (например `192.168.1.1`), порт Entware SSH, логин, пароль Entware (или пусто + `KEENGEN_SSH_KEY`) → **Сохранить** (probe SSH). Подробнее: [пароль или SSH-ключ](docs/howto.md#вход-пароль-или-ssh-ключ).

Пока probe не ок, «Прочитать с Keenetic» выключена. Пароль не уходит на GitHub Pages и не пишется в логи keengen на ПК.

В keengen на Keenetic (`:1001`) эта кнопка **не нужна и отключена** (серая) — UI уже на роутере, SSH-профиль не задаётся.
...
BusyBox Entware **не умеет** `tar -c` — архив `configs.tgz` собирается **на ПК**. Стягивание файлов: SFTP если есть, иначе SSH `find`+`cat` (у Dropbear часто нет SFTP). Установка IPK продолжается, даже если локальный бэкап не удался.
...
> **Архитектура и проверка.** Официальный релизный IPK — только **mipsel-3.4**. Проверенная цель: **Keenetic Hopper** (Entware mipsel-3.4). Другие архитектуры Entware (например **aarch64-3.10**) в Release **не** публикуются и **не** тестировались — без железа для проверки мы их не выкладываем. Проверка установки IPK на конкретном устройстве — у владельца роутера; в CI железа нет.
...
> **Обновление IPK — предпочтительно из keengen на ПК.**  
> Путь с ПК надёжный: HTTPS на ПК → SSH → `opkg` → проверка → старт `:1001`.  
> Обновление **с самой страницы `:1001`** (self-update) мы ужесточили (verify файлов → ответ клиенту → только потом restart), но это тот же процесс, что отдаёт UI: браузер может показать `Failed to fetch` / connection refused. **Роутер не сломан, XKeen не трогали.** Подождите ~20 с и обновите `:1001`, либо откройте keengen на ПК и снова «Установить / Обновить IPK». Откат: с ПК **«Откатить IPK»** / Restore из `backups/`.

1. На ПК: `start.bat` / `python3 start.py`.
2. «Настройка входа» → `root`, LAN-IP роутера, порт `22` → Сохранить.
3. **«Установить IPK…»** / **«Обновить IPK до …»** — всегда качает **GitHub latest** (HTTPS на ПК), бэкап → `opkg` → проверка `:1001`. GitHub API без токена может отвечать 403; тогда используется прямой `releases/latest/download` (опционально `KEENGEN_GITHUB_TOKEN`). Пока идёт установка, во вкладке `:1001` сразу показывается короткий текст («установка…»), затем открывается UI — не пустой `about:blank`. На ПК кнопка **остаётся активной**, даже если версия уже актуальна (принудительная переустановка той же версии — с подтверждением).
4. Строка версий: на ПК — `на ПК · GitHub · на роутере`; на `:1001` — только `на роутере · GitHub` (без «локально» и без подсказки про standalone). Статус входа — только зелёная/красная рамка у «Настройка входа».
5. В keengen на Keenetic (`:1001`) кнопка ставит/обновляет пакет **без SSH**. Если IPK уже актуален — кнопка **выключена** («IPK актуален»); повторная установка только из keengen на ПК.
...
1. Откройте `http://<router>:1001/` — «Настройка входа» будет **серой** (это нормально).
2. Сразу «Прочитать с Keenetic» / «Залить» — работа с `/opt/etc/xray/configs` и `/opt/etc/xkeen`.
