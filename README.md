...
5. В keengen на Keenetic (`:1001`) кнопка ставит/обновляет пакет **без SSH**. Если IPK уже актуален — кнопка **выключена** («IPK актуален»); повторная установка только из keengen на ПК. Откат на предыдущий IPK — только с ПК: **«Откатить на предыдущий IPK»** / **«Откатить IPK»** из слепка `previous-ipk` в `backups/`.
