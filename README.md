...
3. **«Установить IPK…»** / **«Обновить IPK до …»** — всегда качает **GitHub latest** (HTTPS на ПК), бэкап → `opkg` → проверка `:1001`. GitHub API без токена может отвечать 403; тогда используется прямой `releases/latest/download` (опционально `KEENGEN_GITHUB_TOKEN`).
