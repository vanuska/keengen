...
var KG_I18N={
 ru:{
  ...
  ipkConfirmSame:'На роутере уже эта же версия. Переустановить ту же версию с GitHub latest? Будет бэкап, затем opkg.',
  ipkConfirmRollback:'Откатить IPK на предыдущий из слепка ({0})? Пакет на роутере будет заменён.',
  ipkRollback:'Откатить на предыдущий IPK',
  ipkNeedRoot:'для установки с ПК укажите логин root в «Настройка входа»',
  ipkBusy:'ставлю / обновляю IPK…',
  ipkUiLoading:'keengen: установка… сейчас откроется UI',
  ipkUiFail:'keengen: установка не удалась — см. лог в основной вкладке',
  ...
  ipkFailNet:'связь с keengen на ПК оборвалась (helper закрыт, таймаут или сеть). Проверьте start.bat и повторите.',
  ipkFailSelfUpdate:'обновление с :1001 обрывает связь с UI — подождите 20 с и откройте :1001 снова, либо поставьте IPK из keengen на ПК (роутер не сломан)',
  ipkReloading:'IPK {0} установлен — UI перезапускается. Подождите 10–20 с и откройте :1001 снова.',
  ipkVerPc:'на ПК {0} · GitHub {1} · на роутере {2}{3}',
  ipkVerEntware:'на роутере {0} · GitHub {1}',
  ...
  bakHint:'Бэкап конфигов всегда при «Прочитать» и при установке IPK (папка backups/ на этом ПК). «Откатить IPK» — откат на IPK из слепка (previous-ipk), только с ПК под root SSH. «Удалить IPK» только в keengen на ПК, не в keengen на Keenetic (:1001).',
  bakPick:'Слепок',
  bakRefresh:'Обновить список бэкапов',
  bakRestoreCfg:'Restore конфиги',
  bakRestoreIpk:'Откатить IPK',
  ...
  tipBakRestoreIpk:'Откат на IPK из выбранного слепка (previous-ipk или installed)',
  tipIpkRollback:'Откатить на предыдущий IPK из новейшего слепка backups/ (или last-installed.ipk)',
  tipIpkRollbackNone:'Нет локального previous-ipk для отката — сначала установите/обновите IPK с ПК',
  tipIpkCurrentEntware:'IPK уже актуален. Повторная установка только из keengen на ПК',
  tipIpkReinstallPc:'Принудительно переустановить ту же версию с GitHub (бэкап + opkg)',
 ...
 en:{
  ...
  ipkConfirmSame:'Router already has this version. Reinstall the same version from GitHub latest? Will backup, then opkg.',
  ipkConfirmRollback:'Roll back IPK to the previous package from snapshot ({0})? Package on the router will be replaced.',
  ipkRollback:'Roll back to previous IPK',
  ipkNeedRoot:'for PC install set login to root in Login settings',
  ipkBusy:'installing / updating IPK…',
  ipkUiLoading:'keengen: installing… UI will open shortly',
  ipkUiFail:'keengen: install failed — see the log on the main tab',
  ...
  ipkFailNet:'lost connection to keengen on PC (helper closed, timeout, or network). Check start.bat and retry.',
  ipkFailSelfUpdate:'updating from :1001 drops the UI — wait 20 s and reopen :1001, or install IPK from keengen on PC (router is fine)',
  ipkReloading:'IPK {0} installed — UI is restarting. Wait 10–20 s and reopen :1001.',
  ipkVerPc:'on PC {0} · GitHub {1} · on router {2}{3}',
  ipkVerEntware:'on router {0} · GitHub {1}',
  ...
  bakHint:'Config backup always runs on Read and on IPK install (backups/ on this PC). “Roll back IPK” restores the previous-ipk from a snapshot — PC only, root SSH. “Remove IPK” only in keengen on PC — not in keengen on Keenetic (:1001).',
  bakPick:'Snapshot',
  bakRefresh:'Refresh backup list',
  bakRestoreCfg:'Restore configs',
  bakRestoreIpk:'Roll back IPK',
  ...
  tipBakRestoreIpk:'Roll back to IPK from the selected snapshot (previous-ipk or installed)',
  tipIpkRollback:'Roll back to previous IPK from the newest backups/ snapshot (or last-installed.ipk)',
  tipIpkRollbackNone:'No local previous-ipk for rollback — install/update IPK from PC first',
  tipIpkCurrentEntware:'IPK already up to date. Reinstall only from keengen on PC',
  tipIpkReinstallPc:'Force-reinstall the same version from GitHub (backup + opkg)',
